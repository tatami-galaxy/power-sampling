...
from scripts.rl.trace_monitor import MonitoredReward, TraceMonitorCallback
...
def load_deepmath(
    ...
    # Preserve difficulty (float → string bucket) for trace monitoring.
    ds = ds.map(
        lambda x: {
            "problem": x["question"],
            "answer": x["final_answer"],
            "difficulty": str(round(x["difficulty"], 1)) if x.get("difficulty") is not None else None,
        },
...
def format_grpo(example):
    """Format a NuminaMath/Polaris/DeepMath example for GRPO.

    Returns a dict with:
    - prompt: chat messages (system + user) for the model to complete
    - answer: gold answer string (passed to reward function as kwarg)
    - difficulty: (optional) difficulty metadata, passed through to reward function
    """
    prompt = [
...
def train(args):
    ...
    # Normalize metadata to a unified "difficulty" column for trace monitoring.
    # NuminaMath has "source" (e.g. "olympiads"), Polaris/DeepMath have "difficulty".
    if "source" in ds.column_names and "difficulty" not in ds.column_names:
        ds = ds.rename_column("source", "difficulty")

    keep_columns = {"answer"}
    if "difficulty" in ds.column_names:
        keep_columns.add("difficulty")
    ...
    # Wrap reward function with trace monitor
    monitored_reward = MonitoredReward(accuracy_reward)
    trace_callback = TraceMonitorCallback(monitored_reward, num_generations=args.num_generations)

    # Trainer
    trainer = GRPOTrainer(
        model=model,
        args=training_args,
        reward_funcs=monitored_reward,
        train_dataset=ds,
    ...
    has_difficulty = "difficulty" in ds.column_names
    print(f"  Trace monitor:       ON")
    print(f"  Difficulty tracking: {'ON' if has_difficulty else 'OFF (no difficulty column)'}")
    print(f"{'='*60}\n")
